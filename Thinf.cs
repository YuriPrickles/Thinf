	...
	public class Thinf : Mod
	{
		...
		public override void PostSetupContent()
		{
			...
			if (bossChecklist != null)
			{
				...
				"Place Holder Text Sex Among Us Balls Imposter Sussy Baka",
				"Thinf/NPCs/Core/CoreBC",
				"Thinf/NPCs/Core/Core_Head_Boss"
				);
			}
		...
		public override void Load()
		{
			if (!dedServ)
			{
				Ref<Effect> filterRef = new Ref<Effect>(GetEffect("Effects/Drugs"));
				Filters.Scene["Thinf:Drugs"] = new Filter(new ScreenShaderData(filterRef, "Drugs").UseColor(Color.DarkViolet).UseOpacity(0.2f), EffectPriority.VeryHigh);
				Filters.Scene["Thinf:Drugs"].Load();
			}
		...
		public static void Kaboom(Vector2 position)
		{
			...
			if (!dedServ)
			{
				Mod modButCool = ModLoader.GetMod("Thinf");
                Main.PlaySound(modButCool.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/RegularBoom").WithVolume(1.5f), position);
			}
		}
	}
}