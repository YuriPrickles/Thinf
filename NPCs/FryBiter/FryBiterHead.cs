...
namespace Thinf.NPCs.FryBiter
{
    public class FryBiterHead : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Fry Biter");
        }
        public override void SetDefaults()
        {
            npc.lifeMax = 17500;        //this is the npc health
            npc.damage = 70;    //this is the npc damage
        ...
        public override bool PreAI()
        {
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    int randomWormLength = Main.rand.Next(25, 45);
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("FryBiterBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("FryBiterTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
