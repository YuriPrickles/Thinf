...
{
	public class NightmareFeederHead : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			npc.width = 34; //this is where you put the npc sprite width.     important
			npc.height = 38; //this is where you put the npc sprite height.   important
			...
			npc.value = Item.buyPrice(0, 23, 1, 10);
			npc.npcSlots = 1f;
			npc.netAlways = true;
		}
		...
		{
			Item.NewItem(npc.getRect(), ModContent.ItemType<NightmareFuel>(), Main.rand.Next(3) + 4);
		}
		public override bool PreAI()
		{
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				if (npc.ai[0] == 0)
				{
					...
					int randomWormLength = Main.rand.Next(75, 125);
					for (int i = 0; i < randomWormLength; ++i)
					{
						...
						latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<NightmareFeederBody>(), npc.whoAmI, 0, latestNPC);
						Main.npc[(int)latestNPC].realLife = npc.whoAmI;
						Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
					}
					// When we're out of that loop, we want to 'close' the worm with a tail part!
					latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<NightmareFeederTail>(), npc.whoAmI, 0, latestNPC);
					Main.npc[(int)latestNPC].realLife = npc.whoAmI;
					Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
		...
		{
			scale = 3.2f;   //this make the NPC Health Bar biger
			return null;
		}
