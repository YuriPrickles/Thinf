...
namespace Thinf.NPCs
{
	...
	public class FlightKey : ModNPC
	{
		...
		{
			DisplayName.SetDefault("Flight Key");
		}
		...
		{
			if (!NPC.AnyNPCs(mod.NPCType("LightKey")) && !NPC.AnyNPCs(mod.NPCType("NightKey")))
			{
				Main.NewText("The Chest Wasteland grows stronger and messier!", 255, 255, 0);
				downedSoulKeys = true;
			}
			Item.NewItem(npc.getRect(), mod.ItemType("FragmentOfFlight"), Main.rand.Next(10) + 18);
		}
		public override void AI()
		{
			...
			if (corn%10 == 0)
			{
				...
				int type = ProjectileID.HarpyFeather;  //put your projectile
				...
				if (corn == 800)
				{
					...
					NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, NPCID.Harpy);
					NPC wyvern = Main.npc[NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, NPCID.WyvernHead)];
					wyvern.life = 1200;
					wyvern.lifeMax = 1200;
					corn = 0;
				}
