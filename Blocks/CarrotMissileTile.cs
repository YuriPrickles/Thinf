...
namespace Thinf.Blocks
{
	...
	public class CarrotMissileTile : ModTile
	{
		...
		public override void SetDefaults()
		{
			...
			TileObjectData.newTile.HookPostPlaceMyPlayer = new PlacementHook(ModContent.GetInstance<CarrotMissileTileEntity>().Hook_AfterPlacement, -1, 0, true);
		...
		{
			ModContent.GetInstance<CarrotMissileTileEntity>().Kill(i, j);
		}
		...
		public override bool Drop(int i, int j)
		{
			...
			if (stage == PlantStage.Grown)
				Item.NewItem(new Vector2(i, j).ToWorldCoordinates(), ItemID.Carrot, Main.rand.Next(4) + 5);
