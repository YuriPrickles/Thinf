...
namespace Thinf.Blocks
{
    ...
    public class CarrotMissileTileEntity : ModTileEntity
    {
        ...
        public override bool ValidTile(int i, int j)
        {
            ...
            return tile.active() && tile.type == ModContent.TileType<CarrotMissileTile>();
        }
                ...
                if (spikeTimer == 190)
                {
                    for (int i = 0; i < Main.maxNPCs; ++i)
                    {
                        ...
                        if (target.active && !target.friendly && !target.dontTakeDamage && target.Distance(Position.ToVector2().ToWorldCoordinates()) < 800 && target.type != NPCID.TargetDummy)
                        {
                            ...
                            if (spinachBoost)
                            {
                                Projectile projectile = Projectile.NewProjectileDirect(Position.ToVector2().ToWorldCoordinates(), Vector2.Zero, ModContent.ProjectileType<CarrotMissile>(), 52, 7, player.whoAmI);
                            ...
                            {
                                Projectile projectile = Projectile.NewProjectileDirect(Position.ToVector2().ToWorldCoordinates(), Vector2.Zero, ModContent.ProjectileType<CarrotMissile>(), 38, 7, player.whoAmI);
                                projectile.friendly = true;
                                projectile.hostile = false;
                                projectile.magic = false;
                                projectile.minion = false;
