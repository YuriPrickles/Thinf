...
namespace Thinf.Blocks
{
	...
	public class BloodclotTile : ModTile
	{
		...
		public override bool Drop(int i, int j)
		{
			...
			{
				Item.NewItem(new Vector2(i, j).ToWorldCoordinates(), ModContent.ItemType<BloodclotSeeds>(), Main.rand.Next(2, 4));
				Item.NewItem(new Vector2(i, j).ToWorldCoordinates(), ModContent.ItemType<Bloodclot>(), Main.rand.Next(2, 3));
			}
			return false;
		}
		...
		public override void RandomUpdate(int i, int j)
		{
			...
			if (stage != PlantStage.Grown)
			{
				if (stage != PlantStage.Planted && ((Main.bloodMoon || Main.moonPhase == 1) && !Main.dayTime))
