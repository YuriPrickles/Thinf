...
{
    public class SpinachTileEntity : ModTileEntity
    {
        int spikeTimer = 0;
        ...
        public override bool ValidTile(int i, int j)
        {
            ...
            return tile.active() && tile.type == ModContent.TileType<SpinachTile>();
        }
        public override void Update()
        {
            ...
            {
                if (player.Distance(Position.ToVector2().ToWorldCoordinates()) < 16 * 16)
                {
                    player.GetModPlayer<MyPlayer>().spinachStrength = true;
                }
                ...
                for (int i = 0; i < dustSpawnAmount; ++i)
                {
                    ...
                    Dust dust = Dust.NewDustPerfect(Position.ToVector2().ToWorldCoordinates() + dustOffset * (16 * 16), DustID.Grass, null, 0, default, 1.5f);
                    dust.noGravity = true;
                }
            }
        }
