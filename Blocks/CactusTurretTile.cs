...
namespace Thinf.Blocks
{
	...
	public class CactusTurretTile : ModTile
	{
		...
		public override void SetDefaults()
		{
			...
			TileObjectData.newTile.HookPostPlaceMyPlayer = new PlacementHook(ModContent.GetInstance<CactusTurretTileEntity>().Hook_AfterPlacement, -1, 0, true);
		...
		{
			ModContent.GetInstance<CactusTurretTileEntity>().Kill(i, j);
		}
		...
		public override bool Drop(int i, int j)
		{
			...
			if (stage == PlantStage.Grown)
				Item.NewItem(new Vector2(i, j).ToWorldCoordinates(), ItemID.Cactus, Main.rand.Next(4) + 5);
