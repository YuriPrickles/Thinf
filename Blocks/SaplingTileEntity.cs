...
namespace Thinf.Blocks
{
    ...
    public class SaplingTileEntity : ModTileEntity
    {
        ...
        public override bool ValidTile(int i, int j)
        {
            ...
            return tile.active() && tile.type == ModContent.TileType<SaplingTile>();
        }
            ...
            if (stage == PlantStage.Grown)
            {
                ...
                if (spikeTimer == 90)
                {
                    for (int i = 0; i < Main.maxNPCs; ++i)
                    {
                        ...
                        if (target.active && !target.friendly && !target.dontTakeDamage && target.Distance(Position.ToVector2().ToWorldCoordinates()) < 600 && target.type != NPCID.TargetDummy)
                        {
                            ...
                            if (spinachBoost)
                            {
                                for (int p = 0; p < 3; ++p)
                                {
                                    Projectile projectile = Projectile.NewProjectileDirect(Position.ToVector2().ToWorldCoordinates(), Vector2.Zero, ModContent.ProjectileType<SapBall>(), 25, 0, player.whoAmI);
                                    projectile.magic = false;
                                    projectile.minion = false;
                                    projectile.melee = false;
                                    projectile.thrown = false;
                                    projectile.ranged = false;
                                    projectile.velocity = (projectile.DirectionTo(target.Center) * 10f).RotatedByRandom(MathHelper.ToRadians(9));
                                }
                            }
                            else
                            {
                                Projectile projectile = Projectile.NewProjectileDirect(Position.ToVector2().ToWorldCoordinates(), Vector2.Zero, ModContent.ProjectileType<SapBall>(), 10, 0, player.whoAmI);
                                projectile.magic = false;
                                projectile.minion = false;
                                projectile.melee = false;
                                projectile.thrown = false;
                                projectile.ranged = false;
                                projectile.velocity = projectile.DirectionTo(target.Center) * 8f;
                            }
                            break;
                        }
                    }
