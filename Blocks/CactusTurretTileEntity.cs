...
namespace Thinf.Blocks
{
    ...
    public class CactusTurretTileEntity : ModTileEntity
    {
        ...
        public override bool ValidTile(int i, int j)
        {
            ...
            return tile.active() && tile.type == ModContent.TileType<CactusTurretTile>();
        }
                ...
                if (spikeTimer == 35)
                {
                    for (int i = 0; i < Main.maxNPCs; ++i)
                    {
                        ...
                        if (target.active && !target.friendly && !target.dontTakeDamage && target.Distance(Position.ToVector2().ToWorldCoordinates()) < 800 && target.type != NPCID.TargetDummy)
                        {
                            ...
                            if (spinachBoost)
                            {
                                Projectile projectile = Projectile.NewProjectileDirect(Position.ToVector2().ToWorldCoordinates(), Vector2.Zero, ModContent.ProjectileType<CacterusSpike>(), 27, 0, player.whoAmI);
                                projectile.friendly = true;
                                projectile.hostile = false;
                                projectile.velocity = projectile.DirectionTo(target.Center) * 11f;
                            }
                            else
                            {
                                Projectile projectile = Projectile.NewProjectileDirect(Position.ToVector2().ToWorldCoordinates(), Vector2.Zero, ModContent.ProjectileType<CacterusSpike>(), 17, 0, player.whoAmI);
                                projectile.friendly = true;
                                projectile.hostile = false;
                                projectile.velocity = projectile.DirectionTo(target.Center) * 9f;
                            }
                            break;
                        }
                    }
