...
{
    public class NightmareImpaler : ModItem
    {
        int chargeTimer = 0;
        public override void SetStaticDefaults()
        {
            Tooltip.SetDefault("As fast as waking up from a bad dream\nOccasionally shoots a barrage of arrows");
        }
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            if (chargeTimer == 14)
            {
                CombatText.NewText(player.getRect(), Color.Red, "Barrage Ready!");
            }
            if (chargeTimer >= 15)
            {
