...
{
    public class BlasterArm : ModItem
    {
		int rocketCharge = 0;
        public override void SetStaticDefaults()
        {
            Tooltip.SetDefault("Shoots rockets and bullets\nHas a 14% chance to not consume ammo\n'Contains a compartment for cupholders and crepe'");
        }
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
			...
			if (rocketCharge >= 7)
			{
				Projectile.NewProjectile(position.X, position.Y, speedX, speedY, ProjectileID.RocketI, damage * 3, knockBack, player.whoAmI);
				rocketCharge = 0;
			}
            return false;
        }
        ...
        {
            return Main.rand.NextFloat() >= .14f;
        }
