...
{
	public class FenceProjectile : ModProjectile
	{
		//nooooo this isnt OctupusArm.cs, you sussy baka!! > // -- // <
		public float width
		{
			get => projectile.ai[0];
		...
		private const float maxAngleSpeed = 0.02f;
		private const float angleBuffer = (float)Math.PI / 12f;
		public const float minLength = 300f;
		private const float maxLength = 100000f;
		...
		public override void AI()
		{
			...
			if (!npc.active || npc.type != ModContent.NPCType<Poltergate>())
			{
				return;
			}
			projectile.timeLeft = 2;
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			for (int k = 1; k <= numUnits; k++)
			{
				...
				if (k == numUnits)
				{
					image = mod.GetTexture("Projectiles/FenceChain");
				}
