...
namespace Thinf.Projectiles
{
	public class LightBolt : ModProjectile
	{
		...
		{
			DisplayName.SetDefault("Light Bolt");     //The English name of the projectile
		...
		{
			Vector2 tppos = new Vector2(target.position.X + Main.rand.Next(-50, 51), target.position.Y + Main.rand.Next(-50, 51));
			if (!WorldGen.TileEmpty((int)tppos.ToTileCoordinates().X, (int)tppos.ToTileCoordinates().Y))
			{
				if (target.type != NPCID.TargetDummy)
				{
					if (!target.boss)
					target.position = tppos;
				}
			}
		}
