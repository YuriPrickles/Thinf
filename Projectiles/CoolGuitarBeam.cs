...
namespace Thinf.Projectiles
{
	public class CoolGuitarBeam : ModProjectile
	{
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			if (!player.GetModPlayer<MyPlayer>().partyTime && player.GetModPlayer<MyPlayer>().partyCharge <= 7500)
			{
				player.GetModPlayer<MyPlayer>().partyCharge += damage / 25;
			}
		}
        public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Guitar Beam");
		}
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<CoolGuitarBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<CoolGuitarHeldProj>())
			{
				projectile.Kill();
				return;
			}

			if (Main.player[projectile.owner].GetModPlayer<MyPlayer>().partyTime)
			{
				projectile.localNPCHitCooldown = 8;
				MaxDamageMultiplier = 1.9f;
				MaxBeamScale = 4f;
			}
			else
			{
				projectile.localNPCHitCooldown = 10;
				MaxDamageMultiplier = 1.5f;
				MaxBeamScale = 2.3f;
			}
			// Grab some variables from the host Prism.
			Vector2 hostPrismDir = Vector2.Normalize(hostPrism.velocity);
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / CoolGuitarHeldProj.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > CoolGuitarHeldProj.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - CoolGuitarHeldProj.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * CoolGuitarHeldProj.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / CoolGuitarHeldProj.NumBeams) % BeamHueVariance + BeamColorHue;
